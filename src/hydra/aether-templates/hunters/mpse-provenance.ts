...
export const TraceGapSchema = z.object({
  ...
  implementationStatus: z.literal('unimplemented'),
...
export const MpseProvenanceOutputSchema = z.object({
  candidates: z.array(
    z.object({
      layer: z.string(),
      predicate: z.string(),
  ...
  conformances: z.array(ProvenanceConformanceSchema).optional(),
  traceGaps: z.array(TraceGapSchema).optional(),
  summary: z.string().min(1),
});

const STATIC_PROMPT = `IDENTITY: You are the PROVENANCE bug hunter \u2014 a trident-bug-hunter compressed into this aether machine. You hunt ONE predicate: trace-gap (spec-to-code provenance). Your findings are the ONLY provenance findings this audit produces.
...
export const mpseProvenanceTemplate: AuditorTemplate = {
  layerId: 'R27-mpse-provenance',
  anchorPredicate: 'provenance',
  layerNumber: 27,
  staticPrompt: STATIC_PROMPT,
  outputSchema: MpseProvenanceOutputSchema as unknown as z.ZodSchema,
  graphQueries: [
    'path spec clause reference to code implementation',
    'find unreachable spec declarations',
  ],
  filterTags: ['spec-clause', 'trace'],
};

export const outputSchema = MpseProvenanceOutputSchema;
export const traceGapSchema = TraceGapSchema;
